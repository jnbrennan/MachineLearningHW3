...
test = np.array(zip(X_test_std, y_atest))
...
#Use getDistance to calculate all pairwise distances between train and test
...
def main():
    ...
    for x in range(len(X_test_std)):
            neighbours = get_neighbours(training_set=train, test_instance=test[x][0], k=6)
            majority_vote = get_majority_vote(neighbours)
            predictions.append(majority_vote)
 
    #Performance of the classification
    print '\nAccuracy: ' + str(accuracy_score(y_atest, predictions)) + "\n"
    report = classification_report(y_atest, predictions, target_names = iris.target_names)
